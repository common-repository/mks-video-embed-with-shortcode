<?php
class MKS_Form extends MKS_Component{
	...
	public function init(){
		...
		if( is_admin() ){
			wp_enqueue_script('mks_form_colorpicker_admin', $this->component_url.'js/colorpicker.js' , array( 'jquery' ), $this->js_version );
			wp_enqueue_script('mks_form_admin', $this->component_url.'js/mks_form_admin.js' , array( 'jquery' ), $this->js_version );
			
			wp_enqueue_style( 'mks_form_colorpicker_admin', $this->component_url.'css/colorpicker.css', array( ), $this->css_version );
			wp_enqueue_style( 'mks_form_admin', $this->component_url.'css/mks_form_admin.css', array( ), $this->css_version );
			
		}elseif( $this->init_in_front ){
			
			wp_enqueue_script('mks_form_colorpicker', $this->component_url.'js/colorpicker.js' , array( 'jquery' ), $this->js_version );
			wp_enqueue_script('mks_form', $this->component_url.'js/mks_form_admin.js' , array( 'jquery' ), $this->js_version );
			
			wp_enqueue_style( 'mks_form_colorpicker', $this->component_url.'css/colorpicker.css', array( ), $this->css_version );
			wp_enqueue_style( 'mks_form', $this->component_url.'css/mks_form_admin.css', array( ), $this->css_version );
		}
	...
	public static function text_area( $attributes = array() ){
		...
		switch( $attributes['format'] ){
			...
			default:
					...
					$html .= "<textarea ".self::format_attributes( array('id','name','class','style'), array(), $attributes )." ".$attributes['extra'].">".mks_stripslashes( $attributes['value'] )."</textarea>";
	...
	public static function html_area( $attributes = array() ){
		...
		switch( $attributes['format'] ){
			...
			case 'tmce_holder':
					$html .= "<textarea ".self::format_attributes( array('id','name','class'), array(), $attributes )." ".$attributes['extra']." data-type=\"tmce\" style=\"display: none;\" >".mks_stripslashes( $attributes['value'] )."</textarea>";
			...
			case 'tmce':
					...
					ob_start();
					wp_editor( mks_stripslashes( $attributes['value'] ), $attributes['id'], $settings );
	...
	public static function uploader( $attributes = array() ){
		...
		if( mks_comprare_blog_version( '3.4.1' ) >= 0 ){
	...
	public static function format_attribute( $attribute_name, $attribute_key, $attributes, $show_empty = false, $element_type = '' ){
		...
		if( isset( $attributes[ $attribute_key ] ) && strlen( trim( $attributes[ $attribute_key ] ) ) ){
			if( in_array( $element_type, array( 'text_input' ) ) && $attribute_key == 'value' ){
				return ' '.$attribute_name.'="'.mks_htmlentities( mks_stripslashes( $attributes[ $attribute_key ] ) ).'"';
			}else{
