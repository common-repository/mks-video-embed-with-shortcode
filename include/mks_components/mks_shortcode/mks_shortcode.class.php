<?php
class MKS_Shortcode extends MKS_Component{
	...
	public function include_shortcodes(){
		...
		$this->add_default_external_lib( 'css', 'otw-shortcode-preview', $this->component_url.'css/mks_shortcode_preview.css', 'live_preview', 300 );
		...
		include_once( $this->component_path.'shortcodes/mks_shortcodes.class.php' );
		...
		foreach( $this->shortcodes as $shortcode_key => $shortcode_settings ){
			...
			if( !is_array( $shortcode_settings['children'] ) || !count( $shortcode_settings['children'] ) ){
				...
				if( preg_match( "/^import_shortcode_(.*)$/", $shortcode_key ) ){
					$mks_shortcode_key = 'import_shortcode';
				}elseif( preg_match( "/^custom_shortcode_(.*)$/", $shortcode_key ) ){
					$mks_shortcode_key = 'custom_shortcode';
				}elseif( preg_match( "/^widget_shortcode_(.*)$/", $shortcode_key ) ){
					$mks_shortcode_key = 'widget_shortcode';
				}else{
					$mks_shortcode_key = $shortcode_key;
				}
				
				if( !class_exists( 'MKS_ShortCode_'.$mks_shortcode_key ) ){
					
					if( isset( $shortcode_settings['path'] ) ){
						include_once( $shortcode_settings['path'].'shortcodes/mks_shortcode_'.$mks_shortcode_key.'.class.php' );
					}else{
						include_once( $this->component_path.'shortcodes/mks_shortcode_'.$mks_shortcode_key.'.class.php' );
					}
				}
				
				$class_name = 'MKS_ShortCode_'.$mks_shortcode_key;
	...
	public function register_shortcodes(){
		...
		foreach( $this->shortcodes as $shortcode_key => $shortcode_data ){
			...
			if( isset( $shortcode_data['object'] ) ){
				...
				if( isset( $shortcode_data['object']->shortcode_name ) && !empty( $shortcode_data['object']->shortcode_name ) ){
					
					add_shortcode( 'mks_video_shortcode_ifram', array( &$this->shortcodes[ $shortcode_key ]['object'], 'display_shortcode' ) );
	...
	public function init(){
		...
		if( is_admin() ){
			wp_enqueue_script('mks_shortcode_admin', $this->component_url.'js/mks_shortcode_admin.js' , array( 'jquery' ), $this->js_version );
			wp_enqueue_style( 'mks_shortcode_admin', $this->component_url.'css/mks_shortcode_admin.css', array( ), $this->css_version );
			
			add_action( 'admin_footer', array( &$this, 'load_admin_js' ) );
			
			add_action( 'wp_ajax_mks_shortcode_editor_dialog', array( &$this, 'build_shortcode_editor_dialog' ) );
			add_action( 'wp_ajax_mks_shortcode_get_code', array( &$this, 'get_code' ) );
			add_action( 'wp_ajax_mks_shortcode_live_preview', array( &$this, 'live_preview' ) );
			add_action( 'wp_ajax_mks_shortcode_live_reload', array( &$this, 'live_reload' ) );
			add_action( 'wp_ajax_mks_shortcode_preview_shortcodes', array( &$this, 'preview_shortcodes' ) );
			add_action( 'wp_ajax_mks_shortcode_preview_front_shortcodes', array( &$this, 'preview_front_shortcodes' ) );
			...
			if( $this->init_in_front ){
				wp_enqueue_script('mks_shortcode_front', $this->component_url.'js/mks_shortcode_admin.js' , array( 'jquery' ), $this->js_version );
				wp_enqueue_style( 'mks_shortcode_front', $this->component_url.'css/mks_shortcode_admin.css', array( ), $this->css_version );
	...
	public function add_tinymce_plugin( $plugin_array ){
		...
		if( $this->validate_tinymce_button() ){
			$plugin_array[ $this->tinymce_button_key ] = $this->component_url.'/js/mks_shortcode_tinymce_plugin.js';
		}
		return $plugin_array;
	}
			...
			if( isset( $current_page->base ) && ( $current_page->base == 'post' ) && isset( $current_page->post_type ) ){
				...
				switch( $current_page->post_type ){
					...
					case 'page':
						...
						break;
					default:
							if( isset( $this->editor_button_active_for[ 'cpt' ] ) && $this->editor_button_active_for[ 'cpt' ] ){
								$add_button = true;
							}
						break;
				}
	...
	public function load_admin_js(){
		...
		$js .= "mks_shortcode_component = new mks_shortcode_object();";
		$js .= "mks_shortcode_component.shortcodes = ".$this->format_js_shortcodes().";";
		$js .= "mks_shortcode_component.labels = ".json_encode( $this->labels ).";";
		$js .= "mks_shortcode_component.tinymce_button_key='".$this->tinymce_button_key."';";
		$js .= "mks_shortcode_component.wp_url='admin-ajax'";
		$js .= "</script>";
	...
	public function load_front_js(){
		...
		$js .= "mks_shortcode_component.labels = ".json_encode( $this->labels ).";";
		$js .= "mks_shortcode_component.tinymce_button_key='".$this->tinymce_button_key."';";
		$js .= "mks_shortcode_component.wp_url='".get_site_url()."/wp-admin/admin-ajax'";
		$js .= "</script>";
	...
	public function build_shortcode_editor_dialog(){
		...
		if( $shortcode ){
			...
			if( isset( $this->shortcodes[ $_GET['shortcode'] ]['object']->dialog_text ) && strlen( $this->shortcodes[ $_GET['shortcode'] ]['object']->dialog_text ) ){
				$content .= "<div class=\"mks_shortcode_dialog_text\">".$this->shortcodes[ $_GET['shortcode'] ]['object']->dialog_text."</div>";
			}
			...
			$content .= "<tr>";
			$content .= "<td valign=\"top\"><h3 class=\"mks_editor_section_title\">".$this->get_label('Options')."</h3></td>";
			$content .= "<th class=\"mks_empty_head\">&nbsp;</th>";
	...
	public function preview_shortcodes(){
		...
		{
			$result['shortcodes'] = filter_var($_POST['shortcode'], FILTER_SANITIZE_STRING);
			foreach( $result['shortcodes'] as $shortcode_key => $shortcode )
			{
				...
				if( $this->shortcodes[ $shortcode['shortcode_type'] ]['object']->has_preview ){
					...
					$result['shortcodes'][ $shortcode_key ]['preview'] .= '<div class="mks_live_preview_wapper">';
					...
					$result['shortcodes'][ $shortcode_key ]['preview'] .= '<div class="mks_live_preview_wapper"></div>';
				}
			}
		}
	...
	public function live_preview(){
		...
		if( isset( $_POST['shortcode'] ) ){
			...
			echo '<div class="mks_live_preview_shortcode">';
	...
	public function get_code(){
		...
		$attributes = mks_stripslashes( $_POST );
