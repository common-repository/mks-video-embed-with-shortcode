...
mks_shortcode_object.prototype.open_drowpdown_menu = function( append_to ){
	
	this.dropdown_menu = jQuery( '#mks_shortcode_dropdown_menu' );
	
	if( !this.dropdown_menu.size() ){
	
		var links = '<div id=\"mks_shortcode_dropdown_menu\">';
...
mks_shortcode_object.prototype.insert_code = function( shortcode_object ){
	
};

mks_shortcode_object.prototype.init_dropdown_actions = function(){
	...
	with( this ){
		...
		jQuery( 'body' ).click( function( event ){
			...
			if( jQuery( event.target ).parents( '.mce-btn' ).attr( 'data-otwkey' ) == mks_shortcode_component.tinymce_button_key ){
				close_it = false
			};
			if( jQuery( event.target ).parents( '.mce_' + mks_shortcode_component.tinymce_button_key ).size() ){
				close_it = false;
			};
			if( jQuery( event.target ).hasClass( 'otw-column-control-add' ) ){
				close_it = false;
			};
			if( jQuery( event.target ).parents( '#mks_shortcode_dropdown_menu' ).size() ){
				close_it = false;
			};
				...
				if( matches = jQuery( this ).attr( 'class' ).match( /^otw\-shortcode\-dropdown\-action\-([a-zA-Z0-9_\-]+)$/ ) ){
					...
					switch( matches[1] ){
						...
						case 'close':
							...
							break;
						default:
								jQuery.get( wp_url + '.php?action=mks_shortcode_editor_dialog&shortcode=' + matches[1],function(b){
									...
									tb_position = function(){
										...
										mks_setup_html_areas();
									}
									if( typeof( mks_tb_remove ) == 'undefined' ){
										mks_tb_remove = window.tb_remove;
										tb_remove = function(){
											
											mks_close_html_areas();
											mks_tb_remove();
										}
									}
									...
									b=760<b?760:b;
									b-=110; 
									mks_form_init_fields();
									mks_shortcode_editor = new mks_shortcode_editor_object( matches[1] );
									mks_shortcode_editor.preview = shortcodes[ matches[1] ].object.preview;
									mks_shortcode_editor.wp_url = wp_url;
									
									mks_shortcode_editor.init_fields();
									
									mks_shortcode_editor.shortcode_created = function( shortcode_object ){
										insert_code( shortcode_object );
									}
									tb_show( get_label( 'Insert' ) + ' MKS ' + shortcodes[ matches[1] ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
...
mks_shortcode_object.prototype.get_label = function( label ){
...
mks_shortcode_editor_object = function( type ){
					...
					if( editor && editor instanceof tinymce.Editor ) {
						...
						if( jQuery( '#otw-shortcode-element-' + id_matches[1] ).attr( 'data-loaded' ) != 1 ){
							...
							editor.onChange.add( function(){
								...
								mks_shortcode_editor.live_preview();
							} );
						}
...
mks_shortcode_editor_object.prototype.init_html_areas = function(){
...
mks_shortcode_editor_object.prototype.init_action_buttons = function(){
...
mks_shortcode_editor_object.prototype.live_reload = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=mks_shortcode_live_reload&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
mks_shortcode_editor_object.prototype.live_preview = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=mks_shortcode_live_preview&shortcode=' + this.shortcode_type , { 'shortcode': s_code, 'post': post_id }, function( response ){
...
mks_shortcode_editor_object.prototype.shortcode_error = function( errors ){
...
mks_shortcode_editor_object.prototype.get_values = function(){

	v_code = {};
	v_code.shortcode_code = '';
	v_code.shortcode_type = this.shortcode_type;
	
	for( var field in mks_shortcode_editor.fields ){
	
		var matches = false;
		if( matches = field.match( /^otw\-shortcode\-element\-([a-z0-9\_\-]+)$/ ) ){
			
			switch( mks_shortcode_editor.fields[ field ].element_type ){
				
				case 'checkbox':
						
						if( mks_shortcode_editor.fields[ field ].element[0].checked == true ){
							v_code[ matches[1] ] = mks_shortcode_editor.fields[ field ].current_value;
						}else{
							v_code[ matches[1] ] = '';
						}
					break;
				case 'text_area':
						...
						if( ( mks_shortcode_editor.fields[ field ].element.attr( 'data-type' ) == 'tmce' ) && ( mks_shortcode_editor.fields[ field ].element.attr( 'data-loaded' ) == 1 ) ){
							...
							if( tinyMCE.get( 'otw-shortcode-element-' + matches[1] + '_tmce' ) != null ){
								...
								mks_shortcode_editor.fields[ field ].element.val( v_code[ matches[1] ] );
		...
		}else if( field == 'mks_item_id' ){
			v_code.mks_item_id = mks_shortcode_editor.fields[ field ].current_value;
		}
	};
...
mks_shortcode_editor_object.prototype.get_code = function(){
	...
	with( this ){
		...
		jQuery.post( wp_url + '.php?action=mks_shortcode_get_code&shortcode=' + this.shortcode_type , this.code, function( response ){
...
mks_shortcode_editor_object.prototype.init_fields = function(){
	...
	with( this ){
		jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=text]' ).each( function(){
			...
			if( element.attr( 'id' ) ){
				fields[ element.attr( 'id' ) ] = new mks_shortcode_editor_element( 'text_input', element );
			}
		...
		jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=hidden]' ).each( function(){
			...
			if( element.attr( 'id' ) ){
				fields[ element.attr( 'id' ) ] = new mks_shortcode_editor_element( 'hidden_input', element );
			}
		} );
		jQuery( '.otw-shortcode-editor-fields' ).find( 'input[type=checkbox]' ).each( function(){
			...
			if( element.attr( 'id' ) ){
				fields[ element.attr( 'id' ) ] = new mks_shortcode_editor_element( 'checkbox', element );
			}
			element.change( function(){
				live_preview();
			});
		...
		jQuery( '.otw-shortcode-editor-fields' ).find( 'select' ).each( function(){
			...
			if( element.attr( 'id' ) ){
				fields[ element.attr( 'id' ) ] = new mks_shortcode_editor_element( 'select', element );
			}
			element.change( function(){
		...
		jQuery( '.otw-shortcode-editor-fields' ).find( 'textarea' ).each( function(){
			...
			if( element.attr( 'id' ) ){
				fields[ element.attr( 'id' ) ] = new mks_shortcode_editor_element( 'text_area', element );
			}
			element.change( function(){
				live_preview();
			});
...
mks_shortcode_editor_element = function( element_type, element ){
...
mks_shortcode_component = null;
mks_shortcode_editor = null;