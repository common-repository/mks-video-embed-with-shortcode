function mks_sw_add_shortcode( controler ){
	...
	if( id_matches = controler.id.match( "^(.*)otw-sw-add-shortcode$" ) ){
		...
		var wp_url = mks_shortcode_component.wp_url;
		
		var code_object = mks_sw_get_code_object( shortcode_id );
		...
		if( code_object.shortcodes.length == 0 ){
			...
			jQuery.post( 'admin-ajax.php?action=mks_shortcode_editor_dialog&shortcode=' + shortcode_type, { shortcode_object: post_data }, function(b){
								...
								jQuery( "#otw-dialog").find( '#otw-shortcode-btn-insert' ).val( mks_sw_get_label( 'Save' ) );
								jQuery( "#otw-dialog").find( '#otw-shortcode-btn-insert-bottom' ).val( mks_sw_get_label( 'Save' ) );
								...
								mks_shortcode_editor = new mks_shortcode_editor_object( shortcode_type );
								mks_shortcode_editor.preview = mks_shortcode_component.shortcodes[ shortcode_type ].object.preview;
								...
								mks_shortcode_editor.shortcode_created = function( shortcode_object ){
									mks_sw_insert_code( code_object, -1, shortcode_object, shortcode_type );
								}
								tb_show( ' MKS ' + mks_shortcode_component.shortcodes[ shortcode_type ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
							} );
...
function mks_sw_delete( shortcode_id, shortcode_number ){
	...
	mks_sw_save_code_object( code_object );
};
function mks_sw_dislpay_code( container_id ){

	var id_matches = false;
	if( id_matches = container_id.match( "^(.*)otw-sw-code$" ) ){
		
		var code_object = mks_sw_get_code_object( id_matches[1] );
		...
		if( typeof( code_object.shortcodes ) == 'object' ){
			...
			for( var cS = 0; cS < code_object.shortcodes.length; cS++ ){
				...
				display_html = display_html + '<a href="javascript:;" class="otw-sw-remove" onClick="mks_sw_delete( \'' + shortcode_id + '\', ' + cS + ');"><span>' + mks_sw_get_label( 'remove' ) + '</span></a>';
				display_html = display_html + '<a href="javascript:;" class="otw-sw-edit" onClick="mks_sw_settings( \'' + shortcode_id + '\', ' + cS + ');"><span>' + mks_sw_get_label( 'settings' ) + '</span></a>';
...
function mks_sw_save_code_object( code_object ){
	
	jQuery( '#' + code_object.id + 'otw-sw-code' ).val( JSON.stringify( code_object ) );
	
	mks_sw_dislpay_code( code_object.id + 'otw-sw-code' );
};
function mks_sw_get_code_object( shortcode_id ){
...
function mks_sw_settings( shortcode_id, shortcode_number ){
	
	if( typeof( mks_shortcode_component ) != 'object' ){
		return;
	}
	
	wp_url = mks_shortcode_component.wp_url;
	...
	for( var cS = 0; cS < code_object.shortcodes.length; cS++ ){
		...
		if( shortcode_number == cS ){
			...
			jQuery.post( 'admin-ajax.php?action=mks_shortcode_editor_dialog&shortcode=' + shortcode_type, { shortcode_object: post_data }, function(b){
									...
									mks_shortcode_editor.init_fields();
									mks_shortcode_editor.shortcode_created = function( shortcode_object ){
										mks_sw_insert_code( code_object, shortcode_number, shortcode_object, shortcode_type );
									}
									tb_show( ' MKS ' + mks_shortcode_component.shortcodes[ shortcode_type ].title, "#TB_inline?width="+f+"&height="+b+"&inlineId=otw-dialog" );
								} );
...
function mks_sw_insert_code( code_object, shortcode_number, shortcode_object, shortcode_type ){
...
function mks_sw_get_label( string ){
	
	if( typeof( mks_shortcode_component ) == 'object' ){
		return string;
	}
	return mks_shortcode_component.get_label( string );
}